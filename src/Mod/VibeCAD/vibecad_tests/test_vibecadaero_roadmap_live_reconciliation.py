...
BASELINE_SHA = "8611ac881a67b77b777c38f1749880527d2cc956"
BASELINE_SHORT_SHA = BASELINE_SHA[:8]
BASELINE_COMMIT_TIME = "2026-08-29T18:18:57-07:00"
...
def test_live_reconciliation_pins_the_accepted_main_baseline() -> None:
    ...
    assert (
        ...
        f"`{BASELINE_SHA}` on 2026-08-29; repeat per tranche.**"
    ) in governed
    assert (
    ...
    assert (
        f"**Status: Verified complete for accepted source baseline "
        f"`{BASELINE_SHA}` on 2026-08-29; repeat at every implementation "
        f"tranche.**"
    ) in step_zero
